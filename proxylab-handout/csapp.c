...
/* 
 * rio_readline - Robustly read a text line (not buffered)
 *                based on rio_readn
 */
/* $begin rio_readline */
...
int Rio_writen(int fd, const void *usrbuf, size_t n) 
{
...
ssize_t Rio_readline(int fd, void *usrbuf, size_t maxlen) 
{
    ...
    if ((rc = rio_readline(fd, usrbuf, maxlen)) < 0){
        if(errno == ECONNRESET || errno == EPIPE){
            fprintf(stderr, "Rio_readline error: %s\n", strerror(errno));
            return 0;
        }
        unix_error("Rio_readline error");
    }
    return rc;
} 
