...
int allOddBits(int x) {
  int allo = (0xAA | 0xAA<<8 | 0xAA<<16 | 0xAA<<24);
  return !((allo & x) ^ allo);
}
/* 
 * negate - return -x 
...
int isLessOrEqual(int x, int y) {
  int sx = x>>31&1;
  int sy = y>>31&1;
  return (sx&!sy) | (!(sx^sy)&(((~x+y+1)>>31 & 1) ^ 1));
}
//4
/* 
...
int howManyBits(int x) {
  ...
  int base = (x ^ x<<1);
  //int base = (x ^ x>>1)&~(1<<31);
  //最初的base是上面这样写的，但是因为最后一位不能被判断，先改成现在的样子
  //然后为了加深理解又写了注释中使用原base的正确的做法
  //比较遗憾的是我一开始没想到这样二分可以做，下意识的认为这样是不可以的，而且因为90很大选择了逐位解析，然后我写了个90多一点的做法但是怎么都跨不过90这条线...看了别人的代码才意识到是可以的
  b16 = !!(base >> 16) << 4;
  base = base >> b16;
  b8 = !!(base >> 8) << 3;
  base = base >> b8;
  ...
  base = base >> b1;
  //b0 = !!(base);
  return 1+b1+b2+b4+b8+b16;
  //return 1+b1+b2+b4+b8+b16+b0;
}
//float
/* 
...
unsigned floatScale2(unsigned uf) {
  ...
  //差不多也是这个时候意识到了纯位运算也可以的可能性
  //有兴趣的可以去看/c2/2_94.cpp
}
/* 
